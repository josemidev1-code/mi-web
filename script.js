...
const Apps={
 ...
 contact:{title:'Contact',icon:'📬',render:()=>`
   <div class="contact"><h2>LET'S BUILD SOMETHING.</h2><div class="links">
     <a class="clink" href="https://github.com/TU_USUARIO" target="_blank" rel="noopener"><span class="g">🐙</span> GitHub</a>
     <a class="clink" href="https://www.linkedin.com/in/TU_USUARIO" target="_blank" rel="noopener"><span class="g">💼</span> LinkedIn</a>
     <a class="clink" href="mailto:tu_email@ejemplo.com"><span class="g">✉️</span> tu_email@ejemplo.com</a>
...
function buildIcons(){const c=$('#icons');c.innerHTML='';ICONS.forEach(id=>{const a=Apps[id],el=document.createElement('div');
  el.className='icon';el.dataset.id=id;el.tabIndex=0;el.innerHTML=`<span class="glyph">${a.icon}</span><span class="lbl">${a.title}</span>`;
  el.addEventListener('click',()=>{$$('.icon').forEach(i=>i.classList.remove('sel'));el.classList.add('sel');});
  el.addEventListener('dblclick',()=>openWindow(id));el.addEventListener('keydown',e=>{if(e.key==='Enter')openWindow(id);});c.appendChild(el);});}
function buildMenu(){const m=$('#menu__apps');m.innerHTML='';ICONS.forEach(id=>{const a=Apps[id],b=document.createElement('button');
  b.innerHTML=`<span>${a.icon}</span> ${a.title}`;b.onclick=()=>{openWindow(id);$('#menu').classList.add('hidden');};m.appendChild(b);});}
...
/* ========================================================= LOGIN */
const PASSWORD="josemi"; // ← cambia la contraseña aquí
...
function initLogin(){
  ...
  $('#loginForm').addEventListener('submit',e=>{e.preventDefault();
    if(pass.value===PASSWORD){beep();login.classList.add('out');setTimeout(()=>login.classList.add('hidden'),600);boot();}
    else{login.classList.remove('shake');void login.offsetWidth;login.classList.add('shake');err.textContent='Incorrect password.';pass.select();beep();}});
